...
def _append_transcript_summary(lines: List[str]) -> None:
    ...
    if not lines:
        return
    addition = "\n".join(lines)
    _transcript_summary = f"{_transcript_summary}\n{addition}" if _transcript_summary else addition
...
def _append_exchange_summary(lines: List[str]) -> None:
    ...
    if not lines:
        return
    addition = "\n".join(lines)
    _exchange_summary = f"{_exchange_summary}\n{addition}" if _exchange_summary else addition
...
def record_transcript(text: str, source: str) -> None:
    """Record optional text supplied by manual integrations; live audio no longer calls this."""
...
def build_context(current_input: str, mode: str, include_transcripts: bool = True) -> str:
    ...
    if exchanges:
        ...
        for index, exchange in enumerate(exchanges, start=1):
            lines.append(
                f"Previous AI exchange {index} ({exchange.mode}, {exchange.timestamp})\n"
    ...
    if transcripts:
        ...
        sections.append("Recent supplied transcript turns:\n" + "\n".join(lines))

    if current_input:
        sections.append(f"Current selected text or user request:\n{current_input}")
    else:
        sections.append("No current text was provided. Use screenshot and prior manual context when available.")
    return "\n\n".join(sections)

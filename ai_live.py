...
def process_interview_answer(prompt=""):
    ...
    try:
        # Check if we're already processing something
        if overlay and overlay.is_processing:
            print(f"[{datetime.now().strftime('%H:%M:%S')}] ⚠️ Already processing another request, ignoring interview answer", flush=True)
            return
        ...
        if overlay:
            ...
            if not hasattr(overlay, 'current_session_id'):
                overlay.current_session_id = None
            overlay.current_session_id = session_id
            print(f"[{datetime.now().strftime('%H:%M:%S')}] 🎯 Processing interview answer session {session_id}", flush=True)
        
        # Run the API call in a separate thread to avoid blocking the UI
        def api_call_thread():
            try:
                from chat import answer_interview_question
                ...
                if overlay and overlay.desktop_audio_button.isChecked():
                    ...
                    if desktop_audio:
                        ...
                        print(f"[{datetime.now().strftime('%H:%M:%S')}] ⚠️ No desktop audio available", flush=True)
                else:
                    print(f"[{datetime.now().strftime('%H:%M:%S')}] ⚠️ Desktop audio is disabled", flush=True)
                
                # Use the answer_interview_question function with desktop audio only
                response_json = answer_interview_question(desktop_audio)
                
                # Check if this session was canceled before updating UI
                if overlay and hasattr(overlay, 'current_session_id') and overlay.current_session_id != session_id:
                    print(f"[{datetime.now().strftime('%H:%M:%S')}] 🚫 Interview answer session {session_id} was canceled, discarding results", flush=True)
                    return
                ...
                print(f"[{datetime.now().strftime('%H:%M:%S')}] Error in interview answer: {e}", flush=True)
                if overlay:
                    error_response = {"user_query": "Interview Question", "response": f"Error in interview answer: {str(e)}"}
                    QtCore.QMetaObject.invokeMethod(
                        overlay,
                        "update_response",
                        QtCore.Qt.ConnectionType.QueuedConnection,
        ...
        print(f"[{datetime.now().strftime('%H:%M:%S')}] Error setting up interview answer processing: {e}", flush=True)
        if overlay:
            overlay.update_status("Error", "#FF0000")
